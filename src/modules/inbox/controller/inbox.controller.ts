...
export class InboxController {
  ...
  @ApiOperation({
    ...
    description: 'Send a notification to a user from the organization.',
  })
  async sendNotification(
    ...
    const result = await this.inboxService.sendManualReminder(
      currentUser,
      bookingId,
      response,
    );
  ...
  @ApiOperation({
    ...
    description: 'Retrieve inbox messages for the authenticated user.',
  })
  async getInboxMessages(
    @CurrentUser() user: User,
    @Res() response: Response,
    @Query()
    query: { page?: number; limit?: number; is_read?: string; search?: string },
  ) {
    ...
    const result = await this.notificationManager.getInboxMessages(
      user.id,
      query,
    );
  ...
  async markNotificationAsRead(
    ...
    // You can use this.inboxService to call service methods for business logic
    const result = await this.notificationManager.markAsRead(
      user.id,
      notificationId,
    );
    return ResponseUtil.success(result, response);
  ...
  async deleteNotification(
    ...
    // You can use this.inboxService to call service methods for business logic
    const result = await this.notificationManager.deleteNotification(
      user.id,
      notificationId,
    );
    return ResponseUtil.success(result, response);
  ...
  async clearAllNotifications(
    ...
    // You can use this.inboxService to call service methods for business logic
    const result = await this.notificationManager.deleteAllNotifications(
      user.id,
    );
    return ResponseUtil.success(result, response);
  }
