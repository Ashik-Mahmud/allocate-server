...
export class CommunityService {
  ...
  async createPostCommunity(
    createPostCommunityDto: CreatePostCommunityDto,
    ...
    const { isExpired, isFree } = await checkCommunityTrial(
      this.prisma,
      user.org_id,
    );
    if (isFree && isExpired) {
      throw new ForbiddenException(
        'Your free trial for the Community Hub has expired. Please upgrade your plan to continue creating posts.',
      );
    }
  ...
  async updatePostCommunity(
    postId: string,
    updatePostCommunityDto: UpdatePostCommunityDto,
    user: CurrentUserType,
    ip: string,
    agent: string,
  ) {
    ...
    if (isFree && isExpired) {
      throw new ForbiddenException(
        'Your free trial for the Community Hub has expired. Please upgrade your plan to continue updating posts.',
      );
    }

    const result = await this.prisma.$transaction(async (tx) => {
      const oldPost = await tx.communityHub?.findUnique({
        where: { id: postId },
      });
      ...
      if (
        ...
        throw new ForbiddenException(
          'You do not have permission to publish this post as it is currently private. Please make it public first.',
        );
      }
    ...
    const becamePublished =
      (oldPost?.status === CommunityHubStatus.DRAFT &&
        post.status === CommunityHubStatus.PUBLISHED) ||
      (data?.status === CommunityHubStatus.PUBLISHED &&
        post?.isPrivate === false);

    if (!isAdmin && becamePublished) {
      await this.communityRealtimeService.sendBroadcastNotification(
        post,
        user,
        true,
        oldPost?.status === CommunityHubStatus.PUBLISHED ? false : true,
      );
    }
    {
      await this.communityRealtimeService.sendBroadcastNotification(
  ...
  async deletePostCommunity(
    postId: string,
    user: CurrentUserType,
    ip: string,
    agent: string,
    action: { isPermanent: boolean | string } = { isPermanent: false },
  ) {
    ...
    if (isFree && isExpired) {
      throw new ForbiddenException(
        'Your free trial for the Community Hub has expired. Please upgrade your plan to continue deleting posts.',
      );
    }
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (action.isPermanent && action?.isPermanent === 'true') {
        ...
        await tx.communityHub.update({
          where: { id: postId },
          data: { deletedAt: new Date(), status: CommunityHubStatus.ARCHIVED },
        });
      }
  ...
  async restorePostCommunity(
    ...
    if (isFree && isExpired) {
      throw new ForbiddenException(
        'Your free trial for the Community Hub has expired. Please upgrade your plan to continue restoring posts.',
      );
    }
    const result = await this.prisma.communityHub.update({
      where: { id: postId },
      data: { deletedAt: null, status: CommunityHubStatus.DRAFT },
    });
  ...
  async getPostCommunity(
    filters: CommunityPostFilterDto,
    user: CurrentUserType,
  ) {
    ...
    if (user.role === Role.ADMIN) {
      ...
      roleVisibilityCondition = {
        AND: [
          {
            OR: [{ org_id: user.org_id }, { org_id: null }],
          },
          {
            visibility: {
              path: ['showToOrgAdmin'],
    ...
    const where: Prisma.CommunityHubWhereInput = {
      AND: [...baseConditions, roleVisibilityCondition, ...optionalConditions],
    };
  ...
  async getTopCommunityAuthors(user: CurrentUserType) {
    ...
    const { isExpired, isFree, trialEndDate } = await checkCommunityTrial(
      this.prisma,
      user.org_id,
    );
    if (isFree && isExpired) {
    ...
    const acknowledgedPosts = allActivePosts
      .sort((a, b) => {
        const aCount = Array.isArray(a.acknowledgments)
          ? a.acknowledgments.length
          : 0;
        const bCount = Array.isArray(b.acknowledgments)
  ...
  async getPostCommunityById(postId: string, user: CurrentUserType) {
    ...
    if (user.role === Role.ADMIN) {
      ...
      canViewPost =
        user?.id === post.authorId ? true : visibility?.showToAdmin === true;
    } else if (user.role === Role.STAFF) {
      // Staff: Can view if showToStaff is true
      canViewPost =
        user?.id === post.authorId ? true : visibility?.showToStaff === true;
    } else if (user.role === Role.ORG_ADMIN) {
      // Organization Admin: Can view if showToOrgAdmin is true
      // BUT: Cannot view staff-level posts (where author is STAFF)
      canViewPost =
        user?.id === post.authorId ? true : visibility?.showToOrgAdmin === true;
    }
  ...
  async commentOnPostCommunity(
    postId: string,
    comment: CreateCommunityPostCommentDto,
    user: CurrentUserType,
  ) {
    ...
    if (isFree && isExpired) {
      throw new ForbiddenException(
        'Your free trial for the Community Hub has expired. Please upgrade your plan to continue commenting on posts.',
      );
    }
    const post = await this.prisma.communityHub.findUnique({
      where: { id: postId },
    ...
    this.communityRealtimeService.sendCommentUpdatedOnCommunity(
      postId,
      newCommentData as TPostComment,
    );
...
`;
    ...
    if (updatedPost) {
      this.communityRealtimeService.sendCommentUpdatedOnCommunity(post?.id, {
        ...commentToDelete,
        isDeleted: true,
      });
    }
