...
export class StaffService {
  ...
  async createStaff(createStaffDto: CreateStaffDto, user: User, res: Response) {
    ...
    if (
      isSubscriptionLimitReached(currentPlan, 'MAX_USERS', currentUsersCount)
    ) {
    ...
    if (existingUser) {
      if (existingUser.deletedAt !== null) {
        ...
        throw new BadRequestException(
          'User is already active in another organization',
        );
      }
    } else {
  ...
  async getStaffById(id: string, user: User) {
    if (!user?.org_id) {
      throw new ForbiddenException(
        'You must belong to an organization to view staff details',
      );
    }
    const organization = await this.prisma.organizations.findUnique({
    ...
    const isPaidPlan =
      (organization?.plan_type === PlanType.PRO ||
    ...
    return {
      ...staff,
      creditTransactions: shouldShowTransactions
        ? staff?.creditTransactions
        : [],
    };
  }
  ...
  async updateStaff(
    ...
    // Log the activity
    const updatedFields = Object.keys(updateStaffDto).filter(
      (key) => updateStaffDto[key] !== undefined,
    );
    const ipAddress =
      (res?.req?.headers['x-forwarded-for'] as string) ||
      res?.req?.ip ||
  ...
  async deleteStaff(id: string, user: User, res: Response) {
    ...
    if (pendingBookings) {
      throw new BadRequestException(
        'Cannot delete staff member with pending or confirmed bookings. Please resolve the bookings before deletion.',
      );
    }
    ...
    if ((staff.personal_credits || 0) > 0) {
      throw new BadRequestException(
        'Cannot delete staff member with assigned credits. Please revoke or use the credits before deletion.',
      );
    }
  ...
  async manageSingleStaffCredits(
    id: string,
    manageCreditsDto: ManageCreditsDto,
    user: User,
    res: Response,
  ) {
    ...
    const staff = await this.prisma.user.findFirst({
      ...
      include: {
        organization: { select: { name: true, credit_pool: true, id: true } },
      },
    });
    ...
    if (Number(staff?.organization?.credit_pool || 0) < credits) {
      throw new BadRequestException(
        'Insufficient credits in the organization pool',
      );
    }
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Failed to manage credits for the staff member',
      );
    }
  }

  // revoke credits from staff member and return to organization pool
  async revokeStaffCredits(
    id: string,
    manageCreditsDto: ManageCreditsDto,
    user: User,
    res: Response,
    ...
    if (Number(staff?.personal_credits || 0) < credits) {
      throw new BadRequestException(
        'Staff member does not have enough credits to revoke',
      );
    }
  ...
  async manageMultipleStaffCredits(
    manageCreditsDto: ManageMultipleStaffCreditsDto,
    user: User,
    res: Response,
  ) {
    ...
    if (!orgId) {
      throw new ForbiddenException(
        'You must belong to an organization to manage staff credits',
      );
    }
    ...
    const totalRequiredCredits = staffCredits.reduce(
    ...
    if (Number(organization.credit_pool || 0) < totalRequiredCredits) {
      throw new BadRequestException(
        'Insufficient credits in the organization pool for this operation',
      );
    }
    ...
    try {
      const updatedStaffs = await this.prisma.$transaction(async (tx) => {
        ...
        const updates = staffCredits.map(async (sc) => {
          ...
          if (!staff)
            throw new NotFoundException(`Staff ${sc.staff_id} not found`);
      ...
      console.error('Failed to send email:', error);
      throw new InternalServerErrorException(
        'Failed to manage credits for the multiple staff members',
      );
    }
  }
