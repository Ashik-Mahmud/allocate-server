...
export class SalesInquiryController {
    ...
    async createInquiry(
        @Body(new ZodValidationPipe(CreateSalesInquirySchema)) data: CreateSalesInquiryDto, @Res() response: Response
    ) {
    ...
    @Roles(Role.ADMIN)
    @ApiBearerAuth()
    @ApiQuery({ name: 'status', required: false, enum: ['PENDING', 'CONTACTED', 'CLOSED', 'CONVERTED'] })
    @ApiQuery({ name: 'org_id', required: false })
    @ApiQuery({ name: 'country', required: false })
    @ApiQuery({ name: 'search', required: false })
    @ApiQuery({ name: 'sortBy', required: false, enum: ['createdAt', 'name', 'status'] })
    @ApiQuery({ name: 'sortOrder', required: false, enum: ['asc', 'desc'] })
    @ApiQuery({ name: 'page', required: false, type: Number })
    @ApiQuery({ name: 'limit', required: false, type: Number })
    async getAllInquiries(
        @Query(new ZodValidationPipe(SalesInquiryFiltersSchema)) filters: SalesInquiryFiltersDto,
        @CurrentUser() user: CurrentUserType,
        @Res() response: Response
    ): Promise<PaginatedResponse<Partial<SalesInquiry>>> {
        const result = await this.salesInquiryService.getAllInquiries(filters, user);
    ...
    */
    ...
    async updateInquiry(
        @Param('id') id: string,
        @Body(new ZodValidationPipe(UpdateSalesInquirySchema)) data: UpdateSalesInquiryDto,
        @Res() response: Response
    ) {
