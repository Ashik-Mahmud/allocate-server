...
export class ResourcesRuleService {
  ...
  async createResourceRule(
    updateResourcesRuleDto: UpdateResourceRuleDto,
    user: User,
    resourceId: string,
    res?: Response,
  ) {
    ...
    if (!isFeatureAccessible(currentPlan, 'ADVANCED_RULES')) {
      const message = buildSubscriptionAccessErrorMessage(currentPlan, [
        PlanType.PRO,
        PlanType.ENTERPRISE,
      ]);
    ...
    try {
      ...
      await this.sharedService.logActivity(this.prisma, {
        ...
        action:
          newRule.createdAt.getTime() === newRule.updatedAt.getTime()
            ? 'RESOURCE_RULE_CREATE'
            : 'RESOURCE_RULE_UPDATE',
      ...
      throw new InternalServerErrorException(
        error?.message ??
          'An error occurred while creating the resource rule. Please try again later.',
      );
    }
  }
}
