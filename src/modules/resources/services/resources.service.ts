    ...
    try {
      ...
      const finalMetadata = updateResourceDto.metadata
        ? {
            ...
            ...(updateResourceDto.metadata as object),
          }
        : resource.metadata;
  ...
  async markResourceUnderMaintenance(
    ...
    try {
      ...
      if (resource?.is_occupied) {
        throw new ForbiddenException(
          'Resource is currently occupied. Please try again later.',
        );
      }
      // Log resource update activity
      const ipAddress =
  ...
  async deleteResource(user: User, resourceId: string, res?: Response) {
    ...
    try {
      ...
      if (resource._count.bookings > 0) {
        throw new ForbiddenException(
          'Cannot delete resource with upcoming bookings. Please cancel the bookings first.',
        );
      }
