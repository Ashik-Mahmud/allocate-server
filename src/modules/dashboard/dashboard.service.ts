...
export class DashboardService {
  ...
  async getOrganizationOverview(user: User & CurrentUserType) {
    ...
    if (user.role !== Role.ORG_ADMIN) {
      throw new ForbiddenException(
        'Organization overview is available for organization admins only',
      );
    }
    ...
    const totalCreditsAssignedValue = Number(
      totalCreditsAssigned._sum.amount || 0,
    );

    const resourceIds = resourceUsage
    ...
    const resourceMap = new Map(
      resources.map((resource) => [
        resource.id,
        { name: resource.name, type: resource.type, photo: resource.photo },
      ]),
    );
  ...
  async getOrganizationOverviewV2(user: User & CurrentUserType) {
    ...
    const organizationCreditPool = Number(organization?.credit_pool || 0);
    const totalCreditsAssigned = Number(
      totalCreditsAssignedAgg._sum.amount || 0,
    );
    const totalCreditsSpentThisMonth = Number(
      totalCreditsSpentThisMonthAgg._sum.amount || 0,
    );
    const activeStaffThisMonthCount = activeStaffThisMonth.length;
    const utilizationRate =
      totalStaff > 0
        ? Math.round((activeStaffThisMonthCount / totalStaff) * 100)
        : 0;
    ...
    const resourceAnalytics = resourceUsage.map((row) => {
      const bookingRows = recentStaffActivity.filter(
        (activity) => activity.resource.id === row.resource_id,
      );
      const totalHours = bookingRows.reduce((sum, activity) => {
        ...
        return (
          sum +
          this.getBookingDurationMinutes(
            activity.start_time,
            activity.end_time,
          ) /
            60
    ...
    const userIdsForSpenders = topSpenders
      .map((row) => row.user_id)
      .filter(Boolean);
    ...
    const spenderNameMap = new Map(
      spenderUsers.map((entry) => [entry.id, entry.name]),
    );
    ...
    const lowCreditAtZero = lowCreditUsers.filter(
      (staff) => Number(staff.personal_credits || 0) === 0,
    );
    const lowCreditSeverityRatio =
      totalStaff > 0 ? lowCreditUsers.length / totalStaff : 0;
    const creditCoverageRatio =
      totalCreditsAssigned > 0
        ? organizationCreditPool / totalCreditsAssigned
        : 0;

    const mostUsedResource =
      resourceAnalytics.length > 0 ? resourceAnalytics[0].name : null;
    ...
    const mostActiveStaff = Array.from(activeStaffRankingMap.entries())
      .map(([staffId, count]) => {
        const match = recentStaffActivity.find(
          (item) => item.user.id === staffId,
        );
        return {
          staffId,
    ...
    const activityInsights = {
      ...
      financialOverview: {
        ...
        lowCreditAlertsCount: lowCreditUsers.length,
        lowCreditSeverity:
          lowCreditSeverityRatio >= 0.4
            ? 'HIGH'
            : lowCreditSeverityRatio >= 0.2
              ? 'MEDIUM'
              : 'LOW',
  ...
  async getStaffOverview(user: User & CurrentUserType) {
    ...
    const usageHistory = bookings.map((booking) => ({
      ...
      durationMinutes: this.getBookingDurationMinutes(
        booking.start_time,
        booking.end_time,
      ),
    }));
  ...
  async getAdminOverview(user: User) {
    if (user.role !== Role.ADMIN) {
      throw new ForbiddenException(
        'System admin overview is available for admins only',
      );
    }
    ...
    const spendMap = new Map(
      orgSpendAgg.map((row) => [row.org_id, Number(row._sum.amount || 0)]),
    );
    const assignedMap = new Map(
      orgCreditAssignedAgg.map((row) => [
        row.org_id,
    ...
    const top5Organizations = [...organizationsWithStats]
      .sort(
        (a, b) =>
          b.creditConsumption - a.creditConsumption ||
    ...
    const planDistribution = buildPlanDistribution(
      Array.from(planCountsMap.entries()).map(([planType, count]) => ({
        planType,
        count,
      })),
    );
    ...
    for (const point of revenueTrendDaily) {
      ...
      weeklyBuckets.set(
        key,
        Number((weeklyBuckets.get(key) || 0) + point.amount),
      );
    }
      ...
      platformSummary: {
        ...
        totalRevenue: {
          lifetimeCreditSales: Number(
            globalCreditsSoldAgg._sum.price_paid || 0,
          ),
          monthlyCreditSales: Number(
            monthlyCreditRevenueAgg._sum.price_paid || 0,
          ),
  ...
  private getRelativeTimeLabel(date: Date | null | undefined) {
    ...
    const diffMinutes = Math.max(
      1,
      Math.floor((Date.now() - safeDate.getTime()) / 60000),
    );
