...
export class AdminService {
  ...
  async broadcastAnnouncement(
    user: User,
    announcementData: BroadcastAnnouncementDto,
  ) {
    if (user.role !== Role.ADMIN) {
      throw new Error('Unauthorized');
    }

    const { title, message, orgIds, type, metadata, receiverType } =
      announcementData;
    const targetRole = {
      ORG: Role.ORG_ADMIN,
      STAFF: Role.STAFF,
  ...
  async restoreOrganization(user: User, orgId: string, response: Response) {
    ...
    try {
      ...
      const { organization, usersSkipped } = await this.prisma.$transaction(
        async (tx) => {
  ...
  async topUpOrganizationCredits(
    user: User,
    orgId: string,
    data: OrganizationCreditTopUpDto,
    response: Response,
  ) {
    if (user.role !== Role.ADMIN) {
      throw new Error('Unauthorized');
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const isAlreadyExpired = subscription?.end_date
      ...
      if (isAlreadyExpired && !data?.extendDate) {
        throw new BadRequestException(
          'Subscription is already expired. Please provide an extend date to reactivate the subscription.',
        );
      }
      ...
      if (
        ...
        const extendDate = data?.extendDate
          ? new Date(data.extendDate)
          : currentEndDate;
        if (extendDate < currentEndDate) {
          throw new BadRequestException(
            'Extend date must be greater than current subscription end date',
          );
        }
        await tx.subscription.update({
          where: { org_id: orgId },
  ...
  async resetUserPassword(user: User, userId: string, response: Response) {
    ...
    const userToUpdate = await this.prisma.user.findUnique({
      where: { id: userId },
      select: { id: true, name: true, email: true, org_id: true, role: true },
    });
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      await this.shared.logActivity(tx, {
        ...
        metadata: {
          ...
          targeted_user_id: userToUpdate.id,
        },
      });
