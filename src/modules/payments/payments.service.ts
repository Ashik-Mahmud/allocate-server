...
export class PaymentsService {
  ...
  async createCheckoutSession(dto: CreateCheckoutDto, user: CurrentUserType) {
    ...
    if (months === 12) {
      ...
      totalAmount =
        years * pricing.annually + remainingMonths * pricing.monthly;
    } else {
  ...
  async handleStripeWebhook(signature: string, req: any) {
    ...
    if (
      ...
      const { userId, planType, months, currency, org_id, email } =
        session.metadata;
      ...
      const subscription = await this.prisma.$transaction(async (tx) => {
        ...
        const creditsToAdd =
          SUBSCRIPTION_LIMITS[planType as PlanType]?.INITIAL_CREDITS || 0;
        ...
        const currentEndDate = currentSub?.end_date
  ...
  async createSSLCOMMERZSession(dto: CreateCheckoutDto, user: CurrentUserType) {
    ...
    try {
      ...
      if (apiResponse?.GatewayPageURL) {
        ...
        throw new Error(
          apiResponse?.failedreason || 'SSLCommerz initialization failed',
        );
      }
    } catch (error) {
  ...
  async handleSSLIPN(body: any, req: Request) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const monthsCount = parseInt(months, 10) || 1;
      const creditsToAdd = SUBSCRIPTION_LIMITS[planType]?.INITIAL_CREDITS || 0;
      const currentPool = Number(organization?.credit_pool) || 0;
      const frozenCredits = Number(organization?.frozen_credits) || 0;
      const newBalance = currentPool + creditsToAdd + frozenCredits;
  ...
  async validateSSLPayment(
    val_id: string,
  ): Promise<{ isValid: boolean; validationData: any }> {
    if (!val_id) return { isValid: false, validationData: null };
    const { validationApiUrl, store_id, store_password } =
      sslCommerzConfig('sandbox');
  ...
  async convertBDTtoUSD(amountInBDT: number | string): Promise<number> {
    const amount =
      typeof amountInBDT === 'string' ? parseFloat(amountInBDT) : amountInBDT;
  ...
  async activateTrial(user: CurrentUserType) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const trialCredits =
        SUBSCRIPTION_LIMITS[PlanType.TRIAL]?.INITIAL_CREDITS || 0;
