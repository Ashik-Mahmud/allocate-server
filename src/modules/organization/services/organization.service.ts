  ...
  async createOrganization(
    ...
    return await this.prisma.$transaction(async (prisma) => {
      ...
      await this.sharedService.logActivity(this.prisma, {
        ...
        ipAddress:
          (response?.req?.headers['x-forwarded-for'] as string) ||
          response?.req?.ip ||
  ...
  async updateOrganization(
    ...
    const client = await this.prisma.user.findUnique({
      ...
      select: {
        ...
        email: true,
        organization: {
          select: { id: true, name: true, is_active: true, plan_type: true },
        },
      },
    });
    ...
    if (!client || !client.organization || !client?.org_id) {
      throw new Error(
        'Client not found or client is not associated with any organization',
      );
    }

    if (client.org_id !== client?.org_id) {
      throw new ForbiddenException(
        'Client is not authorized to update this organization',
      );
    }
    ...
    if (
      client?.organization?.plan_type === PlanType.FREE &&
      updateOrganizationDto.weeklyReportEnabled === true
    ) {
      throw new ForbiddenException(
