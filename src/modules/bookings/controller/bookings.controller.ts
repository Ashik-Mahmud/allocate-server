...
export class BookingsController {
  ...
  @Post('create')
  @ApiOperation({
    summary: 'Create a booking for a resource (STAFF and ORG_ADMIN roles)',
  })
  ...
  @Patch('status')
  @ApiOperation({
    summary: 'Update booking status (STAFF and ORG_ADMIN roles)',
  })
  @ApiResponse({
    status: 200,
  ...
  async updateBookingStatus(
    ...
    const result = await this.service.updateBookingStatus(
      currentUser,
      bookingId,
      updateBookingStatusDto,
      res,
  ...
  @ApiOperation({
    summary:
      'Update booking details (currently only notes) (STAFF and ORG_ADMIN roles)',
  })
  @ApiResponse({
    status: 200,
  ...
  async updateBookingDetails(
    ...
    const result = await this.service.updateBookingDetails(
      currentUser,
      bookingId,
  ...
  @Patch('reschedule')
  @ApiOperation({
    summary: 'Reschedule a booking to a different time slot (ORG_ADMIN role)',
  })
  ...
  @ApiOperation({
    summary:
      'Get available slots for a resource within a date range (STAFF and ORG_ADMIN roles)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiParam({
    name: 'resourceId',
    type: 'string',
    description: 'ID of the resource to check availability for',
  })
  @ApiQuery({
    name: 'date',
    type: 'string',
    description: 'Date  to check availability for (e.g. "2024-01-01")',
  })
  async getAvailableSlots(
    ...
    const result = await this.service.getAvailableSlotsByResource(
      resourceId,
      date,
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Search by resource name or type',
  })
  @ApiOperation({
    summary:
      "Get all bookings for the current user's organization (STAFF and ORG_ADMIN roles)",
  })
  ...
  async getMyBookingsHistory(
    @CurrentUser() currentUser: User,
    @Query() query: MyBookingsHistoryQueryDto,
    @Res() res: Response,
  ): Promise<PaginatedResponse<Bookings>> {
    const result = await this.service.getMyBookingsHistoryService(
      currentUser,
      query,
    );
  ...
  @Get('all')
  @ApiOperation({
    summary: 'Get all bookings for the organization (ORG_ADMIN role only)',
  })
  @ApiResponse({ status: 200, description: 'Bookings retrieved successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized - Token required' })
  @ApiResponse({
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Filter by date range (e.g. "2024-01-01 to 2024-01-07")',
  })
  async getAllBookings(
    @CurrentUser() currentUser: User,
    @Query() query: AllBookingsQueryDto,
    @Res() res: Response,
  ): Promise<PaginatedResponse<Bookings>> {
  ...
  @ApiOperation({
    summary:
      'Get all bookings for a resource for a specific month and year (STAFF and ORG_ADMIN roles)',
  })
  @ApiResponse({ status: 200, description: 'Bookings retrieved successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized - Token required' })
  @ApiResponse({
  ...
  @ApiParam({
    name: 'resourceId',
    type: 'string',
    description: 'ID of the resource to get bookings for',
  })
  @ApiQuery({
    name: 'month',
    type: Number,
    description: 'Month to filter bookings by (1-12)',
  })
  @ApiQuery({
    name: 'year',
    type: Number,
    description: 'Year to filter bookings by (e.g. 2024)',
  })
  async getBookingsByResourceAndMonth(
    ...
    const result = await this.service.getBookingsByResourceAndMonth(
      currentUser,
      resourceId,
      month,
      year,
  ...
  @ApiOperation({
    summary:
      'Get booking statistics for the organization (ORG_ADMIN role only)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Start date for statistics (e.g. "2024-01-01")',
  })
  @ApiQuery({
    name: 'endDate',
    required: false,
    type: String,
    description: 'End date for statistics (e.g. "2024-01-31")',
  })
  @ApiQuery({
    name: 'groupBy',
    required: false,
  ...
  async getBookingStats(
    @CurrentUser() currentUser: User & CurrentUserType,
    @Query() query: BookingStatsQueryDto,
    @Res() res: Response,
  ) {
