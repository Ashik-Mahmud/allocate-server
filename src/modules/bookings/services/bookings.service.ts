...
export class BookingsService {
  ...
  async createBooking(
    ...
    const { user_id, resource_id, start_time, end_time, notes } =
      createBookingDto;
    const timezone = resolveUserTimezone(currentUser);
    ...
    const [resourceCount, staffCount] = await this.prisma.$transaction([
      ...
      this.prisma.user.count({
        where: { org_id: currentUser?.org_id, deletedAt: null },
      }),
    ]);
    ...
    if (
      ...
      (resourceCount > MAX_RESOURCES || staffCount > MAX_USERS)
    ) {
    ...
    if (startTime < now || endTime <= startTime) {
      throw new BadRequestException(
        'Invalid time slot. Start time must be in the future.',
      );
    }
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      if (!resource || !resource.organization.is_active) {
        throw new NotFoundException(
          'Resource is not available or organization is inactive',
        );
      }
      ...
      if (rules) {
        ...
        if (availableDays && availableDays?.length > 0) {
          if (!availableDays.includes(bookingDay)) {
            throw new BadRequestException(
              `Resource is only available on: ${availableDays.join(', ')}`,
            );
          }
        }
        ...
        if (
          rules.max_booking_hours &&
          hoursDifference > rules.max_booking_hours
        ) {
          const maxBookingHrInMin = rules.max_booking_hours * 60;
          throw new BadRequestException(
            `Maximum booking allowed is ${maxBookingHrInMin} minutes`,
          );
        }
        ...
        if (startTime.getTime() < now.getTime() + minLeadTimeMs) {
          throw new BadRequestException(
            `Must book at least ${(rules.min_lead_time || 0) * 60} minutes in advance`,
          );
        }
      }
      ...
      if (overlappingBooking) {
        throw new BadRequestException(
          'Resource is already booked or in buffer period',
        );
      }
      ...
      const bookingStatus =
        currentUser.role === Role.ORG_ADMIN || resource?.isAutoConfirm
          ? BookingStatus.CONFIRMED
          : BookingStatus.PENDING; // Check if user is STAFF then credits will be deducted from personal credits otherwise from organization credits

      const userTotalRequired = totalCost + pendingCredits;
      if (Number(user.personal_credits || 0) < userTotalRequired) {
        throw new BadRequestException(
          `User has insufficient credits to book this resource. ${pendingCredits > 0 ? ` You have ${pendingCredits} credits pending for confirmation from previous bookings. Please wait for them to be confirmed or reduce the booking duration to fit within your available credits.` : ''}`,
        );
      }
      ...
      if (bookingStatus === BookingStatus.CONFIRMED) {
        if (Number(user.personal_credits || 0) < totalCost) {
          throw new BadRequestException(
            'Not enough credits to confirm this booking',
          );
        }
      ...
      await this.bookingUtilService.handlePostBookingActions(
        ...
        currentUser,
        {
          ...
          activityLog: {
            ...
            details:
              'Booking #{{bookingId}} created for {{resourceName}} by {{actorName}}',
          },
        },
      );
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      if (
        ...
        if (user?.role !== Role.ORG_ADMIN) {
          throw new BadRequestException(
            'Only organization admins can confirm bookings',
          );
        }

        // Check if user has enough credits to confirm this booking
        if (
          Number(booking.user.personal_credits || 0) <
            Number(booking.total_cost || 0) ||
          Number(booking.user.personal_credits || 0) < 0
        ) {
          throw new BadRequestException(
            'Not enough credits to confirm this booking',
          );
      ...
      if (
        ...
        const refundPolicyMs =
          Number(GLOBAL_CONFIG.REFUND_POLICY.FULL_REFUND_IF_CANCELLED_WITHIN) *
          60 *
          60 *
          1000;
        ...
        if (
          status === BookingStatus.CANCELLED &&
          timeDifference < refundPolicyMs
        ) {
          refundMultiplier =
            GLOBAL_CONFIG.REFUND_POLICY.PARTIAL_REFUND_IF_CANCELLED_WITHIN;
        }
      ...
      await this.bookingUtilService.handlePostBookingActions(
        ...
        user,
        {
          ...
          notifications: {
            ...
            orgAdmin: {
              enabled:
                user.role !== Role.ORG_ADMIN || booking.user_id !== user.id,
      ...
      return await tx.bookings.update({
        ...
        data: {
          status: finalStatus,
          ...(finalStatus === BookingStatus.CANCELLED && cancellation_reason
  ...
  async updateBookingDetails(
    ...
    if (
      ...
      throw new BadRequestException(
        'You cannot update a booking that is not pending or confirmed',
      );
    }
  ...
  async rescheduleBooking(
    ...
    if (startTime < now || endTime <= startTime) {
      throw new BadRequestException(
        `Invalid time slot. Start time must be in the future with ${timezone} timezone and end time must be after start time.`,
      );
    }
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (howMuchToPay > 0) {
        ...
        if (Number(user.personal_credits || 0) < userTotalRequired) {
          throw new BadRequestException(
            `Insufficient credits. You need ${howMuchToPay} more credits (Total required with pending: ${userTotalRequired}). But Staff has ${user.personal_credits} credits. Please reduce the booking duration or wait for pending bookings to be confirmed to free up credits.`,
          );
        }
      }
      ...
      const startTimeFormatted = formatInTimezone(startTime, timezone, {
        dateStyle: 'short',
        timeStyle: 'short',
      });
      const endTimeFormatted = formatInTimezone(endTime, timezone, {
        dateStyle: 'short',
        timeStyle: 'short',
      });
      ...
      if (howMuchToPay !== 0) {
        await this.sharedService.createCreditTransaction(tx, {
          ...
          refId: booking.id,
          type:
            howMuchToPay > 0 ? TransactionType.SPEND : TransactionType.REFUND,
          description:
            howMuchToPay > 0
  ...
  async getAvailableSlotsByResource(
    ...
    const resolvedTimezone = resolveTimezone(
      timezone,
      resource.organization?.timezone,
    ...
    const workStart = parseDateTimeInTimezone(
      `${date}T${String(rules?.opening_hours).padStart(2, '0')}:00:00`,
      resolvedTimezone,
    );
    const workEnd = parseDateTimeInTimezone(
      `${date}T${String(rules?.closing_hours || 18).padStart(2, '0')}:00:00`,
      resolvedTimezone,
    );
    ...
    while (currentSlotStart.getTime() + slotDurationMs <= workEnd.getTime()) {
      ...
      const isOccupied = bookings.some((booking) => {
        ...
        return (
          currentSlotStart.getTime() < bEndWithBuffer &&
          currentSlotEnd.getTime() > bStartWithBuffer
        );
      });

      if (!isOccupied) {
        availableSlots.push({
          start: formatInTimezone(currentSlotStart, resolvedTimezone, {
            dateStyle: 'short',
            timeStyle: 'short',
          }),
          end: formatInTimezone(currentSlotEnd, resolvedTimezone, {
            dateStyle: 'short',
            timeStyle: 'short',
          }),
        });
  ...
  async getMyBookingsHistoryService(
    user: User,
    query: MyBookingsHistoryQueryDto,
  ) {
    ...
    try {
      ...
      throw new InternalServerErrorException(
        error?.message || 'Error fetching booking history',
      );
    }
  }
  ...
  async getAllBookings(user: User, query: AllBookingsQueryDto) {
    ...
    try {
      ...
      throw new InternalServerErrorException(
        error?.message || 'Error fetching all bookings',
      );
    }
  }
  ...
  async getBookingsByResourceAndMonth(
    ...
    while (currentLoopDate <= endDate) {
      ...
      try {
        const result = await this.getAvailableSlotsByResource(
          resourceId,
          dateStr,
          timezone,
        );
        ...
        if (slots.length === 0) {
          const isAvailableDay = (rules?.availableDays as string[])?.includes(
            dayName,
          );
  ...
  async getBookingStats(
    ...
    const organization = await this.prisma.organizations.findUnique({
      where: { id: user.org_id, deletedAt: null },
    });
    if (!organization) {
      throw new NotFoundException('Organization not found');
    }
    ...
    if (
      ...
      throw new ForbiddenException(
        `Monthly and Weekly statistics are only available in ${PlanType.PRO} or ${PlanType.ENTERPRISE} plan`,
      );
    }
    ...
    try {
      ...
      if (['day', 'week', 'month'].includes(groupBy)) {
        ...
        const stats = Array.from(grouped.entries()).map(([period, values]) => ({
          ...
          averageCredits:
            values.count > 0 ? (values.totalCost / values.count).toFixed(2) : 0,
        }));
        responseToSend.data = stats;

        // premium insight
        if (organization?.plan_type !== PlanType.FREE && stats.length > 0) {
          const totalCreditsSpent = stats.reduce(
            (acc, stat) => acc + (stat.totalCredits || 0),
            0,
          );
          ...
          const daysRemaining =
            avgDailyBurn > 0 ? Math.floor(creditPool / avgDailyBurn) : 0;

          const totalPeriodBookings = stats.reduce(
            (acc, stat) => acc + stat.bookingsCount,
            0,
          );
          responseToSend.premiumInsight = {
            ...
            summary: {
              ...
              bookingPerDay:
                Number((totalPeriodBookings / stats.length).toFixed(2)) || 0,
            },
          };
        }
      }
      ...
      throw new InternalServerErrorException(
        error?.message || 'Error fetching booking statistics',
      );
    }
  }
}
