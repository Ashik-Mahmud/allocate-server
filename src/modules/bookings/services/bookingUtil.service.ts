...
export class BookingUtilService {
  ...
  async handlePostBookingActions(
    ...
    const notificationType = this.getNotificationTypeBookingStatus(
    ...
    const templateData = {
      ...
      startTime: booking?.start_time
        ? formatInTimezone(booking.start_time, timezone, {
            dateStyle: 'medium',
            timeStyle: 'short',
          })
        : undefined,
      endTime: booking?.end_time
        ? formatInTimezone(booking.end_time, timezone, {
            dateStyle: 'medium',
            timeStyle: 'short',
          })
        : undefined,
    ...
    const defaultStaffMessage = getBookingMessage(
      notificationType,
      'STAFF',
      templateData,
    );
    const defaultAdminMessage = getBookingMessage(
      notificationType,
      'ORG_ADMIN',
      templateData,
    );
    ...
    tasks.push(
      this.notificationManager.send({
        ...
        emailSubject: this.resolveTemplate(
          options?.notifications?.staff?.emailSubject ||
            defaultStaffMessage.title,
          booking,
          currentUser,
          options,
        ),
    ...
    if (
      orgAdmin &&
      !isAdminTheRequester &&
      options?.notifications?.orgAdmin?.enabled !== false
    ) {
      tasks.push(
        this.notificationManager.send({
          userId: orgAdmin.id,
          ...
          title: this.resolveTemplate(
            options?.notifications?.orgAdmin?.title ||
              defaultAdminMessage.title,
            booking,
            currentUser,
            options,
          ),
          message: this.resolveTemplate(
            options?.notifications?.orgAdmin?.message ||
              defaultAdminMessage.message,
            booking,
            currentUser,
            options,
          ),
          emailSubject: this.resolveTemplate(
            options?.notifications?.orgAdmin?.emailSubject ||
              defaultAdminMessage.title,
            booking,
            currentUser,
            options,
    ...
    const shouldCreateCreditTransaction =
      options?.creditTransaction?.enabled ??
      Number(booking.total_cost || 0) > 0;
    ...
    if (shouldCreateCreditTransaction) {
      ...
      const amount = Number(
        options?.creditTransaction?.amount ?? booking.total_cost ?? 0,
      );
      const previousBalance = Number(
        options?.creditTransaction?.previousBalance ??
          booking.user?.personal_credits ??
          0,
      );
      const isRefund =
      ...
      const currentBalance =
        ...
        (isRefund ? previousBalance + amount : previousBalance - amount);
    ...
    const activityAction =
      options?.activityLog?.action ||
      this.getActivityActionByStatus(booking.status as BookingStatus);
